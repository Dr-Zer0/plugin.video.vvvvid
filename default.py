    ...
    for channel in currentGlobalChannels:
        if (channel.id == idChannel):
            for category in channel.categoryList:
                ...
                item['is_playable'] = False
                item['path'] = plugin.url_for('showMovieSingleChannelCategory', idChannel=channel.id,
    ...
    for channel in currentGlobalChannels:
        if (channel.id == idChannel):
            for category in channel.categoryList:
                ...
                item['label'] = str(category.name)
                item['is_playable'] = False
                item['path'] = plugin.url_for('showTvSingleChannelCategory', idChannel=channel.id,
                                              category=str(category.id))
                items.append(item)
    return items
    ...
    for channel in currentGlobalChannels:
        if (channel.id == idChannel):
            for category in channel.categoryList:
                ...
                item['label'] = str(category.name)
                item['is_playable'] = False
                item['path'] = plugin.url_for('showAnimeSingleChannelCategory', idChannel=channel.id,
                                              category=str(category.id))
                items.append(item)
    return items
